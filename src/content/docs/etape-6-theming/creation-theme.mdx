...
Une **IIFE** (Immediately Invoked Function Expression) est un pattern JavaScript qui permet d'exécuter immédiatement une fonction anonyme. C'est essentiel dans Drupal pour éviter la pollution de l'espace global et gérer les dépendances proprement.
...
- **Portée isolée** : Les variables sont locales, pas globales
- **Évite les conflits** : Pas de pollution de `window`
- **Injection de dépendances** : Contrôle explicite des dépendances
- **Mode strict** : Permet d'activer `'use strict'` explicitement
- **Performance** : Code minifié plus efficacement
</Aside>
...
(function($, Drupal, once) {
  ...
  // 1. Variables privées (non accessibles de l'extérieur)
  ...
  // 4. Fonctions publiques (si nécessaire)
...
<Aside type="caution" title="Dépendances Drupal">
Toujours inclure `jQuery`, `Drupal`, et `once` comme paramètres. Ces objets sont passés automatiquement par Drupal.
</Aside>
