...
<Aside type="caution" title="Région 'content' obligatoire">
La région **`content`** est obligatoire dans tous les thèmes Drupal. C'est là que le contenu principal de la page sera affiché.
</Aside>

### 🛒 Régions spécifiques e-commerce
...
example:
  css:
    ...
    # Poids CSS_LAYOUT = -100 : Structure, grille, régions
    layout:
      css/layout/grid.css: {}
      css/layout/regions.css: {}
    ...
    # Poids CSS_STATE = 100 : États (hover, active, focus)
    state:
...
```yaml
# Options avancées pour JavaScript
advanced-js:
  js:
    # JS chargé dans le <head>
    js/header-script.js: { header: true }
    
    # JS déjà minifié (évite la minification Drupal)
...
Depuis Drupal 10.1.2, la clé `version` est obligatoire dans chaque librairie pour éviter les problèmes de cache.
</Aside>

## 🟨 JavaScript dans Drupal

### Structure recommandée : IIFE + Drupal.behaviors
...
```
  ...
  <TabItem label="Avec paramètres">

```javascript
// IIFE avec paramètres (injection de dépendances)
(function($, Drupal, once) {
  'use strict';
  
  // Code protégé dans la portée locale
...
- **Portée isolée** : Variables locales, pas de pollution globale
- **Compatible AJAX** : `Drupal.behaviors` se réexécute après les requêtes AJAX
- **Compatible BigPipe** : Fonctionne avec le chargement progressif Drupal
- **API once()** : Évite la réinitialisation multiple des éléments
- **Injection de dépendances** : `jQuery`, `Drupal`, `once` passés explicitement
</Aside>
...
Pour une explication détaillée des IIFE, `Drupal.behaviors`, et l'API `once`, consultez :
...
Nous approfondirons ces concepts dans le chapitre **Alpine.js** où nous créerons des interactions avancées.
</Aside>
...
<Tabs>
  <TabItem label="Types de hooks utilisés">
...
```php
// Pour un thème nommé "tailstore" :
// hook_preprocess_page → tailstore_preprocess_page
// hook_form_alter → tailstore_form_alter
// hook_theme_suggestions_node_alter → tailstore_theme_suggestions_node_alter
...
<Aside type="tip" title="Rôle des hooks dans les thèmes">
- **Préprocessing** : Préparer/modifier les données avant l'affichage
- **Altération** : Modifier les formulaires et leur comportement  
- **Suggestions** : Proposer des templates alternatifs selon le contexte
</Aside>

<Aside type="note" title="Ressources officielles">
Pour une liste complète des hooks disponibles dans Drupal, consultez :
...
**Évolution des hooks dans Drupal 11 :**
- [Nouveaux hooks avec attributs vs hooks procéduraux](https://dev.to/xwero/new-drupal-hook-attribute-34n0)
</Aside>
...
<Aside type="caution" title="Toujours valider les données">
Toujours vérifier l'existence des champs, leur contenu, et valider les types de données avant utilisation.
</Aside>
...
Ne jamais charger des librairies lourdes (sliders, animations) sur toutes les pages. Utilisez les hooks de preprocessing pour un chargement ciblé.
</Aside>

### ♿ Accessibilité : Régions et landmarks

<Aside type="note" title="Régions doivent correspondre à des landmarks HTML5">
Chaque région Drupal doit être rendue avec le landmark HTML5 approprié dans les templates Twig.

| Région Drupal | Landmark HTML5 | Rôle ARIA | Utilisation |
|---------------|----------------|-----------|-------------|
| `header` | `<header>` | `role="banner"` | En-tête principal (logo, recherche) |
| `primary_menu` | `<nav>` | `role="navigation"` | Navigation principale |
| `content` | `<main>` | `role="main"` | Contenu principal (unique par page) |
| `sidebar` | `<aside>` | `role="complementary"` | Contenu complémentaire (filtres, widgets) |
| `footer_top` | `<footer>` | `role="contentinfo"` | Pied de page |

**Implémentation** : Cela sera fait dans les templates Twig (chapitre suivant).
...
**Avantages** :
- Compréhension rapide du rôle
- Auto-documentation du code
- Support IDE (autocomplétion PHPStorm, VS Code)
</Aside>
...
Pour utiliser les breakpoints définis dans `tailstore.breakpoints.yml`, vous devez activer le module **Breakpoint** du core Drupal :
...
### ✅ Checkpoint : Thème activé et fonctionnel
...
<Aside type="caution" title="Dépannage">
**Si le site affiche une page blanche** :
- Vérifiez la syntaxe YAML (pas de tabulations, espaces uniquement)
- Consultez les logs PHP : `tail -f sites/default/files/php-errors.log`
- Désactivez temporairement le thème : `drush config:set system.theme default olivero -y`

**Si les CSS ne se chargent pas** :
- Vérifiez les chemins dans `tailstore.libraries.yml`
- Videz le cache : `drush cr`
- Désactivez l'agrégation CSS : `/admin/config/development/performance`
</Aside>

## 🎯 Exercices pratiques

### Exercice 1 : Créer le thème minimal
...
3. Allez dans **Structure** → **Block layout** (`/admin/structure/block`)
...
| Étape | Fichier/Action | Statut | Commande de validation |
|-------|----------------|--------|------------------------|
...
<Aside type="tip" title="Sauvegarde recommandée">
Une fois le thème fonctionnel, créez un snapshot :
```bash
