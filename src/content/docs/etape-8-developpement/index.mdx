...
description: Créer le module de panier e-commerce avec htmx et paiement Stripe pour TailStore
sidebar:
  order: 0
...
Cette étape est le **cœur technique** de TailStore. Nous allons développer le module `tailstore_cart` qui gère :
- 🛒 **Panier persistant** : Stockage en session avec support multi-utilisateurs
- ⚡ **Interactions temps réel** : Ajout/suppression via htmx (AJAX sans JavaScript)
- 💳 **Paiement sécurisé** : Intégration Stripe Checkout en mode redirect
- 📧 **Notifications** : Emails de confirmation de commande

Ce module peut être réutilisé et adapté pour d'autres projets e-commerce Drupal.
</Aside>
...
- [x] **Étapes 1-7 terminées** : Structure Drupal, contenus, thème
...
- [x] **HTTP/REST** : Méthodes GET, POST, PATCH, DELETE

<Aside type="caution" title="Niveau technique">
Cette étape nécessite des **bases solides en PHP**. Si vous débutez en POO :
1. Révisez les concepts : classes, interfaces, namespaces
...
Le développement de modules est plus complexe que la configuration.
</Aside>

### Outils nécessaires

- [x] **Compte Stripe** : [Créer un compte test gratuit](https://dashboard.stripe.com/register)
- [x] **Éditeur avec autocomplétion** : PHPStorm, VS Code + PHP Intelephense
- [x] **Xdebug** (recommandé) : Pour debugger le code PHP
...
- `Controller/` : Logique de présentation et réponses HTTP
...
<Aside type="caution" title="⚠️ À éviter">
...
Exceptions acceptées : `.module`, hooks, procéduraux.
</Aside>
...
<Aside type="caution" title="Sécurité Stripe">
**Clés API** :
- ❌ **Jamais** committer les clés dans Git
- ✅ Utiliser des variables d'environnement
- ✅ Clés différentes dev/prod
- ✅ Mode test par défaut en dev
...
Voir section [Stripe Checkout](/etape-8-developpement/stripe/) pour détails.
</Aside>
