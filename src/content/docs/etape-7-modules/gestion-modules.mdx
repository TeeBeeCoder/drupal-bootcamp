...
- ❌ Pas de traçabilité (composer.json/lock)
- ❌ Complexifie les déploiements
</Aside>

### Bonne pratique : Workflow d'installation

<Aside type="tip" title="Process professionnel">
1. **Installer** avec Composer : `composer require drupal/module`
2. **Activer** avec Drush : `drush en module -y`
3. **Configurer** via l'interface ou Drush
4. **Exporter** la configuration : `drush cex -y`
5. **Commiter** : `git add composer.json composer.lock config/` puis `git commit`

Ce workflow garantit la reproductibilité sur tous les environnements.
</Aside>
